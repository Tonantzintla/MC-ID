...
| **JWKS**          | `/api/auth/jwks`      | URL to fetch public signing keys.   |
...
| `profile`        | No      | Access to MC-ID account details (`name`, `accounts`).               |
...
<Aside type="tip">Request `openid` explicitly for ID tokens. Refresh tokens require the app to allow the `refresh_token` grant and the authorization request to include `offline_access`.</Aside>
...
This example sends credentials in the request body. Register the app with `client_secret_post`. If the app uses `client_secret_basic`, send credentials with HTTP Basic authentication instead and omit `client_secret` from the body.
