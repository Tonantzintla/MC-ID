<script lang="ts">
  ...
  const disableContinue = $derived.by(() => {
    ...
    if (!isTainted($tainted?.email) || !isTainted($tainted?.["new-password"]) || !isTainted($tainted?.["confirm-password"])) return true;
    if (requestCodeLoading) return true;
    if (($errors["new-password"]?.length ?? 0) > 0 || ($errors["confirm-password"]?.length ?? 0) > 0 || ($errors.email?.length ?? 0) > 0) return true;
