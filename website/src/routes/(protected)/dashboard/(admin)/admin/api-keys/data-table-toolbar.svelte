<script lang="ts" generics="TData">
  ...
  const rateLimitEnabledCol = $derived(table.getColumn("Rate Limit Enabled"));
...
<div class="flex items-center justify-between gap-2">
  <div class="flex flex-1 items-center space-x-2">
    ...
    {#if rateLimitEnabledCol}
      <DataTableFacetedFilter column={rateLimitEnabledCol} title="Rate Limit Enabled" options={booleanOptions} />
    {/if}

    {#if enabledCol}
      <DataTableFacetedFilter column={enabledCol} title="Enabled" options={booleanOptions} />
    {/if}
