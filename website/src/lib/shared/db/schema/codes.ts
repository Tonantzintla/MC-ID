...
export const verificationCodes = pgTable(
  ...
  (table) => [
    index("mc_user_app_idx").on(table.appApiKeyId, table.mcuserId),
    index("code_app_idx").on(table.appApiKeyId, table.code),
    unique("verification_codes_app_user_unique").on(table.appApiKeyId, table.mcuserId)
  ]
);
