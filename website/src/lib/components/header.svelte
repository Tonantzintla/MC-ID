  ...
  <nav class="fixed z-50 w-full px-2">
    <div
      class={cn([
        ...
        isScrolled && "mt-2 max-w-4xl rounded-2xl border border-border bg-background/50 backdrop-blur-lg lg:px-5"
      ])}>
        ...
        <div class="flex w-full justify-between lg:w-auto">
          <a href={resolve("/")} aria-label="home" class="flex items-center space-x-2">
            {#if mode.current === "dark"}
              <enhanced:img
                src="$lib/assets/MC-ID-White.svg"
                class="pointer-events-none h-5.5 w-auto select-none"
                alt="MC-ID"></enhanced:img>
            {:else}
              <enhanced:img
                src="$lib/assets/MC-ID-BG.svg"
                class="pointer-events-none h-8.5 w-auto select-none"
                alt="MC-ID"></enhanced:img>
            {/if}
          </a>
