<script lang="ts" generics="TData">
  ...
  const { table }: { table: TableType<TData> } = $props();
</script>
...
<div class="flex items-center justify-between px-2">
  ...
  <div class="flex items-center space-x-6 lg:space-x-8">
    ...
    <div class="flex items-center space-x-2">
      ...
      <Button variant="outline" class="p-0" onclick={() => table.previousPage()} disabled={!table.getCanPreviousPage()}>
        <span class="sr-only">Go to previous page</span>
        <ChevronLeftIcon />
      </Button>
      <Button variant="outline" class="p-0" onclick={() => table.nextPage()} disabled={!table.getCanNextPage()}>
