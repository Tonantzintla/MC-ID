...
class PasswordInputState {
  constructor(
    ...
    $effect(() => {
      ...
      if (this.root.passwordState.value !== "" && this.root.strength.score < this.root.opts.minScore.current) {
  ...
  props = $derived.by(() => ({
    "aria-invalid": this.root.strength.score < this.root.opts.minScore.current && this.root.passwordState.tainted && this.root.passwordState.strengthMounted
  }));
}
