...
BEGIN
	IF EXISTS (
		...
		WHERE "provider_id" NOT IN ('credential', 'discord')
	) THEN
		RAISE EXCEPTION 'Better Auth 1.7 migration requires an explicit issuer mapping for every provider_id other than credential and discord';
	END IF;
...
BEGIN
	IF EXISTS (
		...
		RAISE EXCEPTION 'Better Auth 1.7 migration found duplicate account identities';
	END IF;
END $$;--> statement-breakpoint
